...
class Solution(object):
    def isMatch(self, s, p):
    	match = [[0 for x in range(len(s)+1)] for x in range(len(p)+1)] 
    	match[0][0] = True
    	...
    	for i in range(0, len(s)+1):
    		for j in range(0, len(p)+1):
    			...
    			if i!=0:
    				...
    				if j>1 and cur_char == '*':
    					match[i][j] = match[i][j-2] or match[i][j-1] or match[i-1][j] and self.isSame(s[i-1], p[i-2])

    				elif match[i-1][i-1] and self.isSame(s[i-1], cur_char):
    					match[i][j] = True
